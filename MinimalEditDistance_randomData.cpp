...
private:
	fstream file;
	int modeID;
	std::ios_base modeIOS;
public:
	Data(string name,int modeID)
	{
		this->modeID=modeID;
		if(this->modeID==0)
		{
			this->modeIOS==new std::ios_base(std::ios_base::in);
		}else if(this->modeID==1)
		{
			this->modeIOS==new std::ios_base(std::ios_base::out);
			//this->modeIOS=std::ios::out;
			//this->modeIOS=std::ios_base::openmode.out;
		}else
		{
			cout<<"The modeID was not given correctly, it can be passed either 0 or 1."<<endl;
			cout<<"The default mode (modeID==0) is used."<<endl;
			this->modeIOS==new std::ios_base(std::ios_base::in);
			//this->modeIOS=std::ios::in;
		}
		file.open(name,this->modeIOS);
	}
public:	
};
int main()
